...
// Refund policy: full refund if cancelled 24h+ before start_at; otherwise no refund.
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: booking, error: bErr } = await supabase
      .from("bookings")
      .select("id, customer_id, status, start_at, stripe_payment_intent")
      .eq("id", bookingId)
      .single();
    ...
    const eligibleForRefund = hoursUntil >= 24 && !!booking.stripe_payment_intent;
